   ...
   // await this.client.spamMessage(message)
    ...
    if (!this.client.config.member.guildOwners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        message.channel.send(`${message.author}` + "```⛔ Komut kullanımını kötüye kullandığın için engellendi. Açtırmak için ( Tokuchi#1984 / Ewing.#1984 ) kişisine ulaşman gerekiyor.```")
